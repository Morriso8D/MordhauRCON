...
* Automod - mute players for using blacklisted words
...
3. open [config.example.json](/config.example.json) and configure the bots setup. E.g. - if discord isn't required, set "discord" to false
```json 
"bootstrap":{
    "discord": false,
    "command_line": true,
...
```
4. rename [config.example.json](/config.example.json) to ``config.json``
...
## Config 
Please make sure you've followed steps 3 and 4 within [setup](#setup) before altering the config.
